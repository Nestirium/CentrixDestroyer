...
public class AddPerm extends ListenerAdapter {
            ...
            switch (args.length) {
                ...
                case 5:
                    ...
                    if (StatementHandler.hasPermission(user, permission, server, value)) {
                        message.getChannel().sendMessage("User already has this permission!").queue();
                        return;
                    }
                    message.getChannel().sendMessage("Please wait...").queue();
                    StatementHandler.addPermission(user, permission, server, value);
                    message.getChannel().sendMessage("Permission has been added successfully! ✅").queue();
                    break;
                default:
